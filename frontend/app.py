...
API_URL = "https://ai-voice-agent-api-nvw3.onrender.com"
...
openai_key = st.sidebar.text_input(
    "OpenAI API Key (optional)", type="password", value=os.getenv("OPENAI_API_KEY", "")
)
if openai_key:
...
# Header block
st.markdown(
    '<div class="main-header">🛡️ Multilingual Voice Bots & Real-Time Assistant</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">Localized agents for Life Insurance and Consumer Finance featuring real-time compliance, compliance signals, and agent nudges.</div>',
    unsafe_allow_html=True,
)
...
with col_right:
    tab_text, tab_voice, tab_nudge = st.tabs(
        ["🔍 RAG Queries", "🎙️ Localized Voice Bots", "📊 Real-Time Assistant & Nudges"]
    )
    ...
    with tab_text:
        ...
        user_query = st.text_input(
            "Enter your policy question", placeholder="Enter your policy Question."
        )
        ...
        if ask_btn and user_query:
            try:
                with st.spinner("Retrieving facts and synthesizing answers..."):
                    ...
                    try:
                        res = requests.post(
                            f"{API_URL}/ask", json={"question": user_query}, timeout=10
                        )
                    ...
                    if not response:
                        ...
                        if not raw_matches:
                            ...
                            response = {
                                ...
                                "source": raw_matches[0]["metadata"].get(
                                    "source", "Local"
                                ),
                    ...
                    with c1:
                        st.metric(
                            "Confidence Score", f"{response['confidence'] * 100:.1f}%"
                        )
                    with c2:
                    ...
                    if response["url"] and response["url"] != "N/A":
                        st.markdown(
                            f"**Document Link:** [{response['source']}]({response['url']})"
                        )
                    ...
                    for idx, chunk in enumerate(response["retrieved_chunks"]):
                        ...
                        with st.expander(
                            f"Evidence Chunk #{idx + 1} (Relevance: {score * 100:.1f}%)"
                        ):
                            st.write(chunk["content"])
                            st.caption(
                                f"Source: {meta.get('source')} | Page: {meta.get('page')} | Category: {meta.get('category')} | URL: {meta.get('url')}"
                            )
    ...
    with tab_voice:
        ...
        st.markdown(
            "Select a target locale/sector to converse with a custom voice assistant."
        )
        ...
        if st.session_state.voice_active:
            if hasattr(st, "audio_input"):
                audio_file = st.audio_input(
                    "Speak your question...", key=f"mic_{st.session_state.turn_count}"
                )
            else:
        ...
        if st.session_state.voice_active and st.session_state.latest_audio_bytes:
            ...
            try:
                st.audio(
                    st.session_state.latest_audio_bytes,
                    format="audio/mp3",
                    autoplay=True,
                )
            except TypeError:
        ...
        if st.session_state.voice_active and audio_file:
            try:
                ...
                transcribe_res = requests.post(
                    f"{API_URL}/voice/transcribe", files=files, timeout=30
                )
                ...
                chat_res = requests.post(
                    f"{API_URL}/voice/chat", json=chat_payload, timeout=20
                )
                ...
                st.session_state.voice_history.append(
                    {"role": "assistant", "text": chat_data["answer"]}
                )
                ...
                synth_res = requests.post(
                    f"{API_URL}/voice/synthesize", json=synth_payload, timeout=20
                )
    ...
    with tab_nudge:
        ...
        if "live_latest_nudge" not in st.session_state:
            st.session_state.live_latest_nudge = {
                "type": "None",
                "confidence": 0.0,
                "recommendation": "No active suggestions.",
                "priority": "Low",
                "reason": "None",
                "timestamp": "--:--:--",
            }
        ...
        if st.session_state.live_active:
            st.success(
                f"Live Mic Session Active (ID: {st.session_state.live_session_id}) - Language: English"
            )
        else:
        ...
        with g_left:
            ...
            for item in st.session_state.live_history:
                bubble_class = (
                    "bubble-user" if item["role"] == "user" else "bubble-agent"
                )
            ...
            if st.session_state.live_active:
                ...
                if audio_file:
                    try:
                        ...
                        asr_start = time.time()
                        files = {
                            "file": ("input.wav", audio_file.getvalue(), "audio/wav")
                        }
                        transcribe_res = requests.post(
                            f"{API_URL}/voice/transcribe", files=files, timeout=30
                        )
                        ...
                        if transcribe_res.status_code == 200:
                            transcript_text = (
                                transcribe_res.json().get("transcript", "").strip()
                            )
                        else:
                            transcript_text = ""
                        ...
                        if transcript_text:
                            ...
                            agent_answer = chat_res.json().get(
                            ...
                            st.session_state.live_history.append(
                                {"role": "assistant", "text": agent_answer}
                            )
                            ...
                            nudge_result = (
                                st.session_state.nudge_engine.process_transcript(
                                    full_transcript_str,
                                    st.session_state.live_session_id,
                                    st.session_state.live_turn,
                                )
                            )
                            ...
                            st.session_state.live_latest_nudge = nudge_result[
                                "latest_nudge"
                            ]
                            ...
                            st.session_state.live_logs.append(
                                {
                                    ...
                                    "latest_nudge": nudge_result["latest_nudge"].get(
                                        "recommendation", "None"
                                    ),
                                }
                            )
                            ...
                            st.session_state.live_latencies["dashboard"].append(
                                dash_time
                            )

                            # Total E2E Latency
                            st.session_state.live_latencies["e2e"].append(
                                time.time() - e2e_start
                            )
            ...
            if st.session_state.live_audio_bytes:
                ...
                try:
                    st.audio(
                        st.session_state.live_audio_bytes,
                        format="audio/mp3",
                        autoplay=True,
                    )
                except TypeError:
        ...
        with g_right:
            ...
            if latest and latest["type"] != "None":
                badge_color = (
                    "badge-critical"
                    if latest.get("priority") == "Critical"
                    else (
                        "badge-high"
                        if latest.get("priority") == "High"
                        else (
                            "badge-medium"
                            if latest.get("priority") == "Medium"
                            else "badge-low"
                        )
                    )
                )
